...
function bindHome() {
  ...
  supervisorLoginForm.addEventListener("submit", (event) => {
  ...
  supervisorShiftForm.addEventListener("submit", (event) => {
    event.preventDefault();
  ...
  supervisorRunForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const session = appState.supervisorSession;
    if (!session) return;
    const lineId = selectedSupervisorLineId();
  ...
  supervisorDownForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const session = appState.supervisorSession;
    if (!session) return;
    const lineId = selectedSupervisorLineId();
...
function bindForms() {
  ...
  document.getElementById("downtimeForm").addEventListener("submit", (event) => {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(event.currentTarget).entries());
    if (!rowIsValidDateShift(data.date, data.shift)) {
      alert("Date and shift are required.");
...
function renderHome() {
  ...
  if (homeTitle) {
    homeTitle.textContent = isSupervisor ? "Production Line Hub Supervisor Portal" : "Production Line Hub";
  }
  if (headerLogoutBtn) {
